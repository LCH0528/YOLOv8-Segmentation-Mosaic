...
# 1：需要打码的不同类别根据指定类别的颜色或者默认掩膜颜色表中的颜色进行打码，具体的打码颜色根据后面设置的self_mosaic_colors的内容确定。
...
# 以下是self_mosaic_colors的设置例子：
# self_mosaic_colors = [[128, 0.5, 0],
#                       "#FF1493",
#                       "g"]

self_mosaic_colors = ["g"]


# 根据self_mosaic_colors的内容对应的具体打码颜色有以下3种情况：
# (1) 如果self_mosaic_colors只设置了一行，即len(self_mosaic_colors)==1（单色）：只指定了一种颜色，需要对所有需打码的类别打上同一种颜色的码，
# 例如打码区域全白、全红等。
# (2) 如果self_mosaic_colors设置了多行，即len(self_mosaic_colors)>1（多色）：需要打码的不同类别根据指定类别颜色进行打码。
# (3) 如果self_mosaic_colors未设置任何行，即self_mosaic_colors==[[]]（默认）：使用默认掩膜颜色表中的颜色进行打码。
# （具体类别打码颜色根据类别号顺序决定，打码类别序号超过20的(表中20种颜色)循环表中的颜色(颜色序号求模)，例如0号、20号、40号等的打码颜色相同）。


# 以下的两个函数用来将设置的self_mosaic_colors的每行转换成对应的RGB的3个数字构成的列表（每个数字在0~255范围内）

# 用于将self_mosaic_colors的每行转换成对应的RGB的3个数字构成的列表
def RGB_color_mode_conversion(self_mosaic_color):
    if self_mosaic_color[0] in single_color.keys():
        return [x * 255 for x in single_color[self_mosaic_color[0]]]
    elif self_mosaic_color[0] == "#":
        return hex2rgb(self_mosaic_color)
    else:
        return [(255*x if 0.0 <= x <= 1.0 else x) for x in self_mosaic_color]

# 生成并返回self_mosaic_colors对应的每个打码类别对应打码颜色的具体RGB值
def mosaic_colors_generation(self_mosaic_colors):
    mosaic_colors = []
    if self_mosaic_colors == [[]]:
        mosaic_colors = [mask_colors[x] for x in mosaic_index]
    else:
        for i in range(len(mosaic_index)):
            if len(self_mosaic_colors) == 1:
                mosaic_colors.append(RGB_color_mode_conversion(self_mosaic_colors[0]))
            if len(self_mosaic_colors) > 1:
                mosaic_colors.append(RGB_color_mode_conversion(self_mosaic_colors[i]))
    return mosaic_colors


# 打码颜色（具体RGB值）
mosaic_colors = mosaic_colors_generation(self_mosaic_colors)
...
if __name__ == '__main__':
    ...
    # file_paths = ['predict_files/picture/行人4.jpg']
